...
sudo -u www-data php  occ maintenance:install --database $DB --database-host $MYSQL_HOST --database-name $DB_NAME  --database-user $DB_USER_NAME --database-pass $DB_PASSWORD --admin-user $ADMIN_LOG --admin-pass $ADMIN_PASS --data-dir $DATA_DIR
